### typing colored text since 2020

- [**keybored**](https://github.com/ronykax/keybored) - macOS app to bind hotkeys to execute actions.
- [**bob**](https://github.com/ronykax/bob) - my personal AI agent built on OpenAI's agents SDK.
- [**speechie**](https://github.com/ronykax/speechie) - self-hosted inference for NVidia's Parakeet V3
- [**bounce back**](https://github.com/ronykax/bounce-back) - hackathon winning 2D game built in 24 hours.
- [**openbin**](https://github.com/ronykax/openbin) - simple self-hosted pastebin API.
- [**euclase**](https://github.com/ronykax/euclase) - digital version of hichord (pocket audio).
- [**drawcat**](https://github.com/drawcat-lol/app) - collection of poorly drawn cats.
- [**year progress**](https://github.com/ronykax/year-progress) - discord bot tracking the current year's progress.

for inquiries **ronykax@icloud.com**.
